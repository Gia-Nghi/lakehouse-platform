...
def build_spark() -> SparkSession:
    return (
        SparkSession.builder.appName("Lakehouse-Bronze-To-Silver")
...
if PROCESS_DATE:
    RAW_JSON_PATH = f"s3a://lakehouse/bronze/nhatot/date={PROCESS_DATE}/"
else:
    # fallback: đọc toàn bộ prefix trong bucket bronze
    RAW_JSON_PATH = os.getenv("BRONZE_PATH", "s3a://lakehouse/bronze/nhatot/")

SILVER_TABLE = "lakehouse.silver.cleaned"
INGEST_LOG_TABLE = "lakehouse.meta.bronze_to_silver_files"
...
# ============================================================
GGTREND_PATH = os.getenv("GGTREND_PATH", "s3a://lakehouse/bronze/ggtrend/")
GGTREND_SILVER_TABLE = "lakehouse.silver.ggtrend_daily"

def read_ggtrend_json() -> Optional[DataFrame]:
    """Read Google Trends JSON files from S3"""
    print(f"=== GGTREND: Reading JSON from: {GGTREND_PATH} ===")
    try:
        # Read JSON files (each file contains {payload: [...]} structure)
        df_raw = (
            spark.read
            .format("json")
            .option("multiLine", True)
            .load(f"{GGTREND_PATH}*.json")
        )
        
        print(f"✓ GGTREND: Read {df_raw.count():,} raw JSON objects")
        print(f"  Raw schema:")
        df_raw.printSchema()
        
        # Extract payload array and explode to individual records
        if "payload" in df_raw.columns:
            df_ggtrend = (
                df_raw
                .select(F.explode(F.col("payload")).alias("data"))
                .select(F.col("data.*"))  # Flatten the nested struct
            )
            print(f"✓ GGTREND: Exploded payload → {df_ggtrend.count():,} individual records")
            print(f"  Flattened schema:")
            df_ggtrend.printSchema()
            print(f"  Columns: {df_ggtrend.columns}")
            return df_ggtrend
        else:
            print(f"⚠ ERROR: 'payload' column not found in JSON structure")
            print(f"  Available columns: {df_raw.columns}")
            return None
        return df_ggtrend
    except AnalysisException as e:
        print(f"!!! Cannot read Google Trends JSON from: {GGTREND_PATH}")
        print(f"  Error: {e}")
        return None
...
def process_ggtrend_data(df: DataFrame) -> DataFrame:
    """Process and standardize Google Trends data"""
    ...
    # Add specific ggtrend column renames (keywords + location)
    ggtrend_col_mapping = {
        "batongsantphcm": "bat_dong_san_tp_hcm",
        "chungcutphcm": "chung_cu_tp_hcm",
        "canhotphcm": "can_ho_tp_hcm",
        "muanhatphcm": "mua_nha_tp_hcm",
        "atnentphcm": "dat_nen_tp_hcm",  # đất nền
    }
    ...
    if "ispartial" in [c.lower() for c in df.columns]:
        ...
        if ispartial_col:
            ...
            df = df.filter((F.col(ispartial_col) == False) | (F.col(ispartial_col) == "false"))
            after_count = df.count()
            print(f"  Filtered isPartial: {before_count:,} → {after_count:,} records (removed {before_count - after_count:,} partial entries)")
    ...
    # Convert data types for numeric columns if they exist
    for col_name in df.columns:
        col_type = df.schema[col_name].dataType
        
        # Convert string columns that should be numeric
        if isinstance(col_type, StringType) and col_name in ["value", "count", "score", "visits"]:
            print(f"  Converting {col_name}: StringType → DoubleType")
            df = df.withColumn(col_name, F.col(col_name).cast(DoubleType()))
        
        # Convert date columns to datetime (TimestampType)
        elif isinstance(col_type, StringType) and "date" in col_name.lower():
            print(f"  Converting {col_name}: StringType → TimestampType")
