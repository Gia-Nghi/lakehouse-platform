...
# ============================================================
spark = build_spark("Lakehouse-Bronze-To-Silver")
...
def load_ggtrend_to_silver(df: DataFrame) -> None:
    """Load processed Google Trends data to Silver layer with idempotent append."""
    ...
    available_dedup_keys = [c for c in dedup_keys if c in df_silver.columns]

    df_silver = df_silver.dropDuplicates(available_dedup_keys)

    print(f"  Output columns ({len(ggtrend_silver_cols)}): {sorted(ggtrend_silver_cols)}")
    print(f"  Dedup keys: {available_dedup_keys}")

    if table_exists(GGTREND_SILVER_TABLE):
        print(f"  Table exists. Checking existing Google Trends records...")

        df_existing = (
            spark.table(GGTREND_SILVER_TABLE)
            .select(*available_dedup_keys)
            .distinct()
        )
        ...
        df_new = df_silver.join(
            df_existing,
            on=available_dedup_keys,
            how="left_anti",
        )

        rows_new = df_new.count()
        rows_skipped = rows_before - rows_new

        print(f"  Incoming records: {rows_before:,}")
        print(f"  New records: {rows_new:,}")
        print(f"  Skipped duplicated records: {rows_skipped:,}")

        if rows_new > 0:
            df_new.writeTo(GGTREND_SILVER_TABLE).append()
            print(f"  ✓ Appended {rows_new:,} new Google Trends records")
        else:
            print("  ✓ No new Google Trends records to append")
